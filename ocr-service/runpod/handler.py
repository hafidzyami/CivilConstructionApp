...
Supports all engines: surya, paddle, hybrid, vlm
...
def process_ocr(input_data):
    ...
    if engine not in ['surya', 'paddle', 'hybrid', 'vlm']:
        return {"error": f"Invalid engine: {engine}. Must be surya, paddle, hybrid, or vlm"}
    ...
    try:
        ...
        # Handle VLM engine - send image directly to VLM
        if engine == 'vlm':
            vlm = get_vlm_engine()
            ...
            if file_type == 'pdf':
                ...
                for page_idx, cv_img in enumerate(images):
                    # Save page as temp image for VLM
                    ...
                    vlm_result = vlm.process_image(page_b64)
                ...
                # Single image
                vlm_result = vlm.process_image(image_b64)
        ...
        # Handle images and PDFs with traditional OCR engines
        images_to_process = []
        if file_type == 'pdf':
...
print("Engines: surya, paddle, hybrid, vlm")
print("=" * 60)
